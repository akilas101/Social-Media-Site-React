...
import { faPencilAlt, faCaretUp, faCaretDown, faWifi,faEnvelopeOpenText,  faPenSquare,faShare,faTrashAlt, faTh  } from '@fortawesome/free-solid-svg-icons'
...
const QuestionDispatch=(id)=>{
    ...
    useEffect(() => {
     ...
     return (
        <div className="homescreen-qcard">
            ...
            </div>{console.log(props.question._id+" from HomeScreen")}
            <NavLink to ={"/SelectedQuestions/"+props.question._id}
             target="_blank"
            >
              <h6 className="card-title"><strong>{props.question.question}</strong></h6>
              </NavLink>
              <NavLink exact to={"/Interests/questions/"+props.question.interest._id}
          
            >  {(props.question.interest)?
                <h5 style={{fontSize:"13px"}} >#{props.question.interest.name}</h5>:
                null
}  </NavLink>  
