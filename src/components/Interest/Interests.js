...
useEffect(()=>{
  Axios.get('/api/interests/'+props.match.params.id)
  .then((res)=>{
     if(res){
    //  alert(res.data)
  ...
  const fetchActivites = () =>{
    // console.log(props.match.params.id+"2345y54we")
    Axios.get('/api/interests/questions/'+props.match.params.id)
    .then((res)=>{
...
if(activity?.type==="question"){
  ...
  return <div key={index} className="posts">
    ...
    <div className="homepage">
    {
      ...
      <div> 
          <div className="interestTitle"> Questions and answers posted in {interest}</div>
          {renderActivites}
